...
def load_image_and_metadata(index, csv_path, image_dir, transform, processor):
    ...
    #row = df.iloc[index]
    filename = "WhatsApp Image 2025-06-04 at 18.53.58_fbfb31df.jpg"
    ...
    plt.axis("off")
    plt.savefig(f"vacation/{filename}_image.png", dpi=300)
    plt.close()

    inputs = processor(images=image_pil, return_tensors="pt")
    gt_coords = torch.tensor([float(0.0), float(0.0)])
    return inputs["pixel_values"].squeeze(0), gt_coords, filename
...
def visualize_prediction(index, model, device):
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(f"vacation/{filename}_predictions.png", dpi=300)
    plt.show()
    plt.close()
