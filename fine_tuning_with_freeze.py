...
class GeoDataset(Dataset):
    def __init__(self, csv_path, image_dir, transform=None):
        ...
        self.normalize = transforms.Normalize(#similar to CLIP
    ...
    def __getitem__(self, idx):
        ...
        return inputs, torch.tensor([lat, lon], dtype=torch.float32)
...
def run(save_model_name, freeze=False, epochs = 10, lr = 1e-4, batch_size = 16, threshhold=16, name_reuse_model=False, K=6):
    ...
    log_dir = os.path.join("logs2", save_model_name)
    writer = SummaryWriter(log_dir=log_dir)


    if freeze == False:
        clip_model = CustomViT()#we want the standard values specified above
        model = StreetCLIPRegressor(clip_model, K, batch_size, device).to(device)

        if name_reuse_model is not None:
            old_model = os.path.join("logs2", name_reuse_model, f"_final.pt")
    ...
    train_dataset = GeoDataset(train_data_path, image_dir, train_transform)
    val_dataset = GeoDataset(val_data_path, image_dir, val_transform)
    ...
    for epoch in range(epochs):
        ...
        if epoch&3==0:
            save_path = os.path.join("logs2", save_model_name, f"_{epoch}_iter.pt")
    ...
    save_path = os.path.join("logs2", save_model_name, f"_final.pt")
    torch.save(model.state_dict(), save_path)
    writer.close()
...
if __name__ == "__main__":
    ...
    save_model_name = "Multigaus-Our-model-augmentation-V1"
    ...
    save_model_name_fine = "Multigaus-Our-model-augmentation-Finetuned-V1"
