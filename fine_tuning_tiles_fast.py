...
def eval_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for imgs, target_class in tqdm(dataloader, desc="Validation", leave=False):
            ...
            correct_preds += (preds == target_class).sum().item()
...
def run(save_model_name, freeze=False, epochs = 10, lr = 1e-4, batch_size = 16, threshhold=16, name_reuse_model=False, K=6):
    ...
    train_dataset = TileDataset(train_data_path, image_dir, processor, train_transform)
    val_dataset = TileDataset(val_data_path, image_dir, processor, val_transform)
    ...
    for epoch in range(epochs):
        ...
        if freeze==False:
            ...
            print(f"[{epoch+1}/{epochs}] Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.2f} | Val Acc: {val_acc:.4f}")
...
if __name__ == "__main__":
    ...
    batch_size = 22
    lr = 5e-5
    epochs = 25 #stops after the first 30steps of the first epoch
    freeze=False
    threshhold = None #about 10 miles
    save_model_name_fine = "Classifier-V2-Finetuned"
